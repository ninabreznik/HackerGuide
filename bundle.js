...
/*------------------------------------------
                FONT
-------------------------------------------*/
var links = ['https://fonts.googleapis.com/css?family=Ubuntu', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css'];
var font = yo(_templateObject, links[0]);
var fontAwesome = yo(_templateObject, links[1]);
...
/*------------------------------------------
                CSS
-------------------------------------------*/
var css = csjs(_templateObject2, BLUEISH, FONT, BLUEISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, imageBasic, BLUEISH, WHITISH, REDISH);
/*------------------------------------------
              HTML
-------------------------------------------*/
  ...
  var showHide = {
    list: function list(data) {
      ...
      return array.map(function (x) {
        return yo(_templateObject4, css.itemButton, x.link, x.name);
      });
    },
    show: function show(data) {
      var newEl = yo(_templateObject5, title, css.button, function (_) {
        return routes(routeHide)(data);
      }, css.items, showHide.list(data));
      var el = document.body.querySelector('.' + title);
      yo.update(el, newEl);
    },
    hide: function hide(data) {
      var newEl = yo(_templateObject6, title, css.button, function (_) {
        return routes(routeShow)(data);
      }, css.title, title);
      var el = document.body.querySelector('.' + title);
      yo.update(el, newEl);
    }
...
function menuButtonComponent(data, title, route, image) {
  var css = csjs(_templateObject7, FONT, image, WHITISH, WHITISH, WHITISH, WHITISH, WHITISH, BLUEISH);

  function template() {
    return yo(_templateObject8, title, css.button, function (_) {
...
module.exports = function router () {
  var ROUTES = {}
  return function routes (name, callback) {
    if (!callback) return ROUTES[name] || function () {}
